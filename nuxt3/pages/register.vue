...
async function submitForm() {
  ...
  try {
    const response = await $fetch('/api/addUsers', { //eu faço chamada pra minha api interna do nitro server, mas pode fazer pra api externa tbm
      method: 'POST',
      body: {
        name: name.value,
    ...
    if (response) {
      if (response.success){
        ...
        responseERROR.value = ``;
        responseOK.value = `Usuário inserido com sucesso`;
      }
    ...
    console.log(error);
    responseOK.value = ``;
    responseERROR.value = 'Erro ao enviar o formulário 2';
  }
}
</script>
